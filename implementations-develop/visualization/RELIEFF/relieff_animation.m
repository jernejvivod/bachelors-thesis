function [rank, weights] = relieff_animation(data, m, k, dist_func, plot, axes, fgr, pauseBtnHandle, resetBtnHandle, timeout, use_deletions)
    ...
    try  % try/catch statement that will catch errors occuring when closing app.
        ...
        for rank = idx_sampled
            ...
            if plot
                ...
                hT = title(axes, {'ReliefF Algorithm Visualization', sprintf('$$ weights = [%.3f, %.3f, %.3f] $$', weights(1), weights(2), weights(3))},'interpreter','latex');
                set(hT, 'FontSize', 17);
            end
            % ### /PLOTTING ###
