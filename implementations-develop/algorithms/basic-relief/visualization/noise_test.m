...
figure; hold on; title(sprintf('Relief Weights for Monotonic and Non-monotonic Feature\nwith Respect to Number of Noise Features'));	
xlim([0, lim_noise]); ylim([-0.1, 1]); xlabel('Number of Noise Features'); ylabel('Weight');
pause(0.5);
for k = 0:lim_noise
	[~, weights] = relief_animation([noisy_data(:, 1:2+k), target], size(data, 1), @(a, b) minkowski_dist(a, b, 2), 0);
	res(k+1, 1) = weights(1);
	res(k+1, 2) = weights(2);
...
figure; title(sprintf('Relief Weights for Monotonic and Non-monotonic Feature\nwith Respect to Number of Noise Features'));
