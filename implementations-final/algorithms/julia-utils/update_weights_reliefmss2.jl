function update_weights(data, e, closest_same, closest_other, weights, weights_mult, m, k, max_f_vals, 
	...
	# Penalty term
	penalty = (abs.(e .- closest_same)./(max_f_vals .- min_f_vals .+ eps(Float64))) - dm_vals_same
	penalty[.!features_msk_same] .= 0.0
	penalty = sum(penalty, dims=1)

	# Reward term
	reward = weights_mult .* (abs.(e .- closest_other)./(max_f_vals .- min_f_vals .+ eps(Float64))) - dm_vals_other
