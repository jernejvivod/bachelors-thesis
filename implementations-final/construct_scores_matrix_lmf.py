...
GROUP_IDX = "ReliefF_ITML1"  # Results index

algs = OrderedDict([
    ('ReliefF', Relieff(k=PARAM_K, dist_func=lambda x1, x2 : np.sqrt(np.sum(np.abs(x1-x2)**2.0, 1)))),
    ('ReliefF_ITML', Relieff(k=PARAM_K, dist_func=lambda x1, x2 : np.sqrt(np.sum(np.abs(x1-x2)**2.0, 1)), learned_metric_func=itml.get_dist_func))
])
