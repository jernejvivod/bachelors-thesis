...
def relief(data, target, m, distance_measure):
    ...
    max_f_vals = np.amax(data, axis=0)              # Get maximum and minimum values of each feature
    ...
    for idx in sample_idxs:                         # Evaluate features using a sample of m examples.
        ...
        for k in np.arange(data.shape[1]):                                   # Go over features
            ...
            # Update weights
            weights[k] = weights[k] - diff(k, e, nearest_hit, isinstance(e[k], float), max_f_vals[k], min_f_vals[k])/m + \
