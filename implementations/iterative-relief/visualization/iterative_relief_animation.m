function [idx, dist_weights] = iterative_relief_animation(data, target, m, min_incl, dist_func, max_iter, plot, timeout, use_deletions)
	...
	while iter_count < max_iter && ~convergence
		...
		feature_weights = zeros(1, size(data, 2) - 1);
		...
		for idx = idx_sampled
			...
			% !!! data_same contains e at index where there is a 0 in
			% dist_same !!!
			...
			for t = 1:size(data, 2)
				w_miss = (0.1 - (dist_other.^2/min_r^2)); w_miss = w_miss.*(w_miss >= 0);
				w_hit = (0.1 - (dist_same.^2/min_r^2)); w_hit = w_hit.*(w_hit >= 0);
				
				numerator1 = sum(abs(e - data_other) .* w_miss, 1);
				denominator1 = sum(w_miss);
				
				numerator2 = sum(abs(e - data_same) .* w_hit, 1);
				denominator2 = sum(w_hit);
