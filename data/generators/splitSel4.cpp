...
int main(int argc, char *argv[])
{
  ...
  int NoDiscrete = NoInfAttr + NoRandAttr + 4 + 4 ;
  ...
  for (int i=0; i < NumberOfExamples ; i++)
  {
     ...
     for (j=0 ; j < NoInfAttr ; j++)
       sum += D[j] ;
     if (sum % 2 == 0)
       Function = D[NoInfAttr] && D[NoInfAttr+1] || D[NoInfAttr+2] && D[NoInfAttr+3] ;
     else
       Function = D[NoInfAttr+4] && D[NoInfAttr+5] || D[NoInfAttr+6] && D[NoInfAttr+7] ;
